...
const email = process.env.INDEED_EMAIL || "romainschaff2612@gmail.com";
const mdpIndeed = process.env.INDEED_PASSWD || "romaindu26";
const website = "https://www.indeed.com";
const jobSearch = process.env.INDEED_SEARCH || "alternance développeur web";
const numOfPages = process.env.INDEED_NUMPAGES || 1;
const locationSearch = process.env.INDEED_LOCATION || "";
const myContractType = process.env.INDEED_CONTRACT_TYPE || "";
const remoteOrNot = process.env.INDEED_REMOTE_WORK_OPTION || "";
...
(async () => {
  ...
  if (email && mdpIndeed) {
    ...
    if ((await loginButtons.count()) > 0) {
      ...
      await passwordInput.type(mdpIndeed, { delay: getRandomInt(50, 150) });
